...
ifeq ($(TARGET_FUNKEY),1)
  OD_TOOLCHAIN ?= /opt/FunKey-sdk-1.1.0/
...
ifneq ($(ENABLE_OPENGL)$(ENABLE_OPENGL_LEGACY),00)
  ...
  ifeq ($(TARGET_LINUX),1)
	ifeq ($(TARGET_RS97),1)
	  ...
	  GFX_CFLAGS  += $(shell /opt/FunKey-sdk-1.1.0/arm-funkey-linux-musleabihf/sysroot/usr/bin/sdl-config --cflags)
	  GFX_LDFLAGS += $(shell /opt/FunKey-sdk-1.1.0/arm-funkey-linux-musleabihf/sysroot/usr/bin/sdl-config --libs) -Wl,--as-needed -Wl,--gc-sections -Wl,-O1,--sort-common -flto -s
